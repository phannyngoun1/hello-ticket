...
export interface {{EntityName}}DetailProps {
  ...
  editable?: boolean;
  onEdit?: (data: {{EntityName}}) => void;
  {{#if hasStatusActions}}
  onActivate?: (data: {{EntityName}}) => Promise<void>;
  onDeactivate?: (data: {{EntityName}}) => Promise<void>;
  onLock?: (data: {{EntityName}}) => Promise<void>;
  onUnlock?: (data: {{EntityName}}) => Promise<void>;
  {{/if}}
  customActions?: (data: {{EntityName}}) => React.ReactNode;
}
...
export function {{EntityName}}Detail({
  ...
  const displayName = useMemo(
    () =>
      data
        ? {{#if hasDisplayName}}get{{EntityName}}DisplayName(){{else}}data.id{{/if}}
        : "",
    [data{{#if hasDisplayName}}, data?.{{#fields}}{{#if isDisplayField}}{{#if isFirst}}{{name}}{{/if}}{{/if}}{{/fields}}{{/if}}]
  );

  {{#if hasStatus}}
  const is{{EntityName}}Active = data?.status === "active";
  {{#if hasLockedUntil}}
