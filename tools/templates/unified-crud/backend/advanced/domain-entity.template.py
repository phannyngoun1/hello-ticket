"""{{EntityName}} aggregate for {{ModuleName}} - Advanced CRUD with complex relationships."""
from __future__ import annotations

from datetime import datetime, timezone
from typing import Optional, Dict, Any{{OptionalDictImport}}{{DateTimeImport}}
...
class {{EntityName}}(AggregateRoot):
    """Represents a {{EntityNameLower}} - advanced entity with complex relationships and business logic."""
...
{{UpdateAssignments}}
    ...
    def _validate_code(self, code: Optional[str]) -> Optional[str]:
        """Validate {{EntityNameLower}} code format."""
        if code is None:
            return None
        if not code or not code.strip():
            raise ValidationError("{{EntityName}} code cannot be empty if provided")
        code = code.strip().upper()
        if len(code) > 100:
            raise ValidationError("{{EntityName}} code cannot exceed 100 characters")
        return code

    def _validate_name(self, name: str) -> str:
        """Validate {{EntityNameLower}} name."""
...
{{NormalizeMethods}}
    def _validate(self) -> None:
        """Validate {{EntityNameLower}} data and business rules."""
{{ValidationMethods}}
        pass
