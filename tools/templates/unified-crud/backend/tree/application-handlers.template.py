...
{{CodeGenerationLogic}}
        ...
        {{EntityNameLower}} = {{EntityName}}(
            ...
            code=code_value,
            name=command.name,
            parent_{{EntityNameLower}}_id=command.parent_{{EntityNameLower}}_id,
...
{{CreateEntityFields}}
    ...
    async def handle_update_{{EntityNameLower}}(self, command: Update{{EntityName}}Command) -> {{EntityName}}:
        ...
        update_kwargs = {}
        if command.parent_{{EntityNameLower}}_id is not None:
            {{EntityNameLower}}.set_parent(command.parent_{{EntityNameLower}}_id)
        if command.sort_order is not None:
            {{EntityNameLower}}.update_sort_order(command.sort_order)
{{UpdateEntityFields}}
    ...
    async def handle_delete_{{EntityNameLower}}(self, command: Delete{{EntityName}}Command) -> bool:
        ...
        logger.info("Deleted {{EntityNameLower}} %s for tenant=%s", command.{{EntityNameLower}}_id, tenant_id)
        return True
...
class {{EntityName}}QueryHandler:
    ...
    async def handle_get_{{EntityNameLower}}_tree(self, query: Get{{EntityName}}TreeQuery):
        tenant_id = require_tenant_context()
        tree = await self._{{EntityNameLower}}_repository.get_{{EntityNameLower}}_tree(tenant_id)
    ...
    async def handle_get_{{EntityNameLower}}_children(
        self, query: Get{{EntityName}}ChildrenQuery
    ) -> List[{{EntityName}}]:
        return await self._{{EntityNameLower}}_repository.get_children(
            query.{{EntityNameLower}}_id
        )

    def _filter_active_nodes(self, nodes: List[{{EntityName}}]) -> List[{{EntityName}}]:
        """Filter out inactive nodes (while keeping active descendants)."""
        filtered: List[{{EntityName}}] = []
        for node in nodes:
