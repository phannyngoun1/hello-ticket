...
router = APIRouter(prefix="/{{EntityNamePluralKebab}}", tags=["{{ModuleName}}"])
...
async def create_{{EntityNameLower}}(
    ...
    current_user: AuthenticatedUser = Depends(RequirePermission(Permission.{{Permission}})),
...
async def list_{{EntityNamePluralSnake}}(
    ...
    current_user: AuthenticatedUser = Depends(RequireAnyPermission([Permission.VIEW_{{PermissionPrefix}}, Permission.{{Permission}}])),
    mediator: Mediator = Depends(get_mediator_dependency),
):
