 ...
 *
 * Dialog component for managing organizer tags with combobox for selecting existing tags or creating new ones
 *
 * @author Phanny
 */
...
export function OrganizerTagsDialog({
  ...
  }, [organizer.id, tagService, searchQuery]);
  ...
  const hasChanges = useMemo(() => {
    const initialTags = organizer.tags || [];
    if (selectedTags.length !== initialTags.length) return true;
    return selectedTags.some((tag) => !initialTags.includes(tag));
  }, [selectedTags, organizer.tags]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[650px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Add or remove tags for <span className="font-medium">{organizer.name || organizer.code}</span>. 
