/**
 * Edit VenueType Dialog Component
 *
 * Full-screen dialog for editing existing venue-types with form validation
 */
...
import { UpdateVenueTypeInput, VenueType} from "./types";

export interface EditVenueTypeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  vt: VenueType | null;
  onSubmit: (id: string, data: UpdateVenueTypeInput) => Promise<void>;
  loading?: boolean;
  error?: Error | null;
}
...
export function EditVenueTypeDialog({
  open,
  onOpenChange,
  vt,
  onSubmit,
  ...
  // Populate form when vt changes or dialog opens
  useEffect(() => {
    if (open && vt) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.code) {
      errors.code = t(
        "pages.settings.ticketing.vt.codeRequired",
        "Code is required"
      );
    }
    ...
    if (!formData.name) {
      errors.name = t(
        "pages.settings.ticketing.vt.nameRequired",
        "Name is required"
      );
    }
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const updateData: UpdateVenueTypeInput = {
      ...
      await onSubmit(vt.id, updateData);
  ...
  }, [open, showConfirmDialog, loading, formData, vt]);
  ...
  return (
    <>
      <FullScreenDialog
        ...
        onClose={handleClose}
        title={t(
          "pages.settings.ticketing.vt.editTitle",
          "Edit VenueType"
        )}
        maxWidth="500px"
        loading={loading}
        ...
        footer={[
          ...
          {
            label: loading
              ? t("pages.settings.ticketing.vt.updating", "Updating...")
              : t("pages.settings.ticketing.vt.update", "Update VenueType"),
            variant: "default",
            type: "submit",
            onClick: (e) => {
              e?.preventDefault();
            ...
            <div className={cn("grid grid-cols-1", density.gapForm)}>
              ...
              <FormItem className={density.spacingFormItem}>
                <FormLabel htmlFor="code" className={cn(density.textSizeLabel, "font-medium")}>
                  {t("pages.settings.ticketing.vt.code", "Code")} *
                </FormLabel>
                <FormControl>
                  <Input
                    id="code"
                    ...
                    disabled={loading}
                    placeholder={t(
                      "pages.settings.ticketing.vt.codePlaceholder",
                      "Enter code"
                    )}
                  />
                </FormControl>
              ...
              <FormItem className={density.spacingFormItem}>
                <FormLabel htmlFor="name" className={cn(density.textSizeLabel, "font-medium")}>
                  {t("pages.settings.ticketing.vt.name", "Name")} *
                </FormLabel>
                <FormControl>
                  <Input
                    id="name"
                    ...
                    disabled={loading}
                    placeholder={t(
                      "pages.settings.ticketing.vt.namePlaceholder",
                      "Enter name"
                    )}
                  />
                </FormControl>
      ...
      <ConfirmationDialog
        ...
        title={t(
          "pages.settings.ticketing.vt.confirmUpdateTitle",
          "Confirm VenueType Update"
        )}
        description={
          <>
            <p className={cn("mb-3", density.textSizeSmall)}>
              {t(
                "pages.settings.ticketing.vt.confirmUpdateDescription",
                "Are you sure you want to update this venue-type?"
              )}
            </p>
          </>
        }
        confirmAction={{label: loading
              ? t("pages.settings.ticketing.vt.updating", "Updating...")
              : t(
                  "pages.settings.ticketing.vt.confirmUpdate",
                  "Confirm & Update"
                ),
