/**
 * Create CustomerType Dialog Component
 *
 * Full-screen dialog for creating new customer-types with form validation
 *
 * @author Phanny
 */
...
import { CreateCustomerTypeInput } from "./types";
import { useCustomerTypeService } from "./customer-type-provider";

export interface CreateCustomerTypeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: CreateCustomerTypeInput) => Promise<void>;
  loading?: boolean;
  error?: Error | null;
}
...
export function CreateCustomerTypeDialog({
  open,
  onOpenChange,
  onSubmit,
  loading = false,
  error = null,
}: CreateCustomerTypeDialogProps) {
  ...
  const handleConfirmSubmit = async () => {
    try {
      const createData: CreateCustomerTypeInput = {
  ...
  return (
    <>
      <FullScreenDialog
        ...
        onClose={handleClose}
        title={t(
          "pages.settings.sales.ct.createTitle",
          "Create CustomerType"
        )}
        maxWidth="500px"
        loading={loading}
        ...
        footer={[
          ...
          {
            label: loading
              ? t("pages.settings.sales.ct.creating", "Creating...")
              : t("pages.settings.sales.ct.create", "Create CustomerType"),
            variant: "default",
            type: "submit",
            onClick: (e) => {
              e?.preventDefault();
      ...
      <ConfirmationDialog
        ...
        title={t(
          "pages.settings.sales.ct.confirmCreateTitle",
          "Confirm CustomerType Creation"
        )}
        description={
          <>
            <p className={cn("mb-3", density.textSizeSmall)}>
              {t(
                "pages.settings.sales.ct.confirmCreateDescription",
                "Are you sure you want to create a new customer-type?"
              )}
            </p>
          </>
        }
        confirmAction={{label: loading
              ? t("pages.settings.sales.ct.creating", "Creating...")
              : t(
                  "pages.settings.sales.ct.confirmCreate",
                  "Confirm & Create"
                ),
