/**
 * Edit Test Dialog Component
 *
 * Full-screen dialog for editing existing tests using shared form logic.
 *
 * @author Phanny
 */
...
import { TestForm, type TestFormData } from "./test-form";
import type { Test, UpdateTestInput } from "./types";

export interface EditTestDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (id: string, data: UpdateTestInput) => Promise<void>;
  test: Test | null;
  title?: string;
  maxWidth?: string;
}
...
export function EditTestDialog({
  ...
  const [pendingFormData, setPendingFormData] = useState<TestFormData | null>(null);
  ...
  const buildPayload = useMemo(() => {
    return (data: TestFormData): UpdateTestInput => ({
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      await onSubmit(test.id, payload);
