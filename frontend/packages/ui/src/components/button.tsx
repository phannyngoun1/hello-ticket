...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    // Apply density-aware height overrides
    let densityHeightClass = "";
    if (densityStyles) {
      if (size === "default" || size === undefined) {
        densityHeightClass = densityStyles.buttonHeight;
      } else if (size === "sm") {
        densityHeightClass = densityStyles.buttonHeightSmall;
      } else if (size === "icon") {
        densityHeightClass = densityStyles.buttonHeight;
      }
      // lg size keeps its original height
    }
    ...
    return (
      <Comp
        className={cn(
          ...
          densityHeightClass && "[&]:h-auto", // Reset height to allow density override
          densityHeightClass,
          className
        )}
        ref={ref}
