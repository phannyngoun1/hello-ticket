 ...
 * Edit Item Dialog
 *
 * Dialog for editing existing inventory items with comprehensive form
 *
 * @author Phanny
 */
...
export interface EditItemDialogProps {
  ...
  onSubmit: (id: string, data: UpdateItemInput) => Promise<void>;
  item: Item | null;
...
export function EditItemDialog({
  ...
  // Helper method to get form data from item (reusable for clearing form)
  const getFormDataFromItem = (): ItemFormData => {
    if (!item) {
      return {
        code: "",
        sku: "",
    ...
    return {
      ...
      uom_mappings: item.uom_mappings || [],
  ...
  const handleClear = () => {
    // Remount form with fresh default values by incrementing formKey
    setFormKey((prev) => prev + 1);
  };

  const handleFormSubmit = async (data: ItemFormData) => {
    if (!item) return; // Show confirmation dialog instead of submitting directly
    setPendingFormData(data);
    setShowConfirmDialog(true);
  };

  const handleConfirmSubmit = async () => {
    if (!item || !pendingFormData) return;
      ...
      >
        ...
        >
          <ItemForm
            ref={formRef}
            key={`${item.id}-${formKey}`}
            defaultValues={getFormDataFromItem()}
          ...
          pendingFormData ? (
            <div>
              <p className={cn("mb-3", density.textSizeSmall)}>
                Are you sure you want to update this item with the following
                information?
              </p>
