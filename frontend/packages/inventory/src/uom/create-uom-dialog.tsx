/**
 * Create UoM Dialog Component
 *
 * Full-screen dialog for creating new units of measure with form validation
 *
 * @author Phanny
 */
...
import { CreateUnitOfMeasureInput } from "../types";
import { useUoMService } from "./uom-provider";
import { useUoM } from "./use-uom";
import { getDefaultBaseUOMs } from "../config/seed-data";
import { useToast } from "@truths/ui";
import { Sparkles } from "lucide-react";

export interface CreateUoMDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: CreateUnitOfMeasureInput) => Promise<void>;
  loading?: boolean;
  error?: Error | null;
}
...
export function CreateUoMDialog({
  ...
  const handleConfirmSubmit = async () => {
    try {
      const createUoMData: CreateUnitOfMeasureInput = {
  ...
  const handleInitializeBaseUOMs = async () => {
    setIsInitializing(true);
    try {
      const defaultUOMs = getDefaultBaseUOMs();
      let successCount = 0;
      let errorCount = 0;

      // Create all default base UOMs sequentially
      for (const uom of defaultUOMs) {
        try {
          await onSubmit(uom);
          successCount++;
        } catch (err) {
          console.error(`Error creating UOM ${uom.code}:`, err);
          errorCount++;
        }
      }
      ...
      if (errorCount === 0) {
        toast({
          ...
          description: t(
            "pages.settings.inventory.uom.baseUomsInitialized",
            "Base units of measure initialized successfully"
          ),
        });
        ...
        toast({
          ...
          description: t(
            "pages.settings.inventory.uom.baseUomsPartiallyInitialized",
            `Initialized ${successCount} of ${defaultUOMs.length} base units of measure`
          ),
          variant: "default",
        });
      } else {
        throw new Error("All initializations failed");
      }

      setShowInitDialog(false);
    } catch (error) {
      console.error("Error initializing base UOMs:", error);
      toast({
        title: t("common.error", "Error"),
        description: t(
          "pages.settings.inventory.uom.initializationError",
          "Failed to initialize base units of measure"
        ),
        variant: "destructive",
      });
    } finally {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape" && (showConfirmDialog || showInitDialog)) {
        e.preventDefault();
        e.stopPropagation();
  ...
  }, [open, showConfirmDialog, showInitDialog, loading, formData]);
  ...
  // Get base UOM options - include current code as option if it's a new base unit
  const baseUomOptions = [
    ...availableUoMs.map((uom) => ({
      value: uom.code,
      label: `${uom.code} - ${uom.name}`,
    })),
    // Allow self-reference if code is entered
    ...(formData.code.trim() &&
    !availableUoMs.some(
  ...
  return (
    <>
      <FullScreenDialog
        ...
        onClose={handleClose}
        title={t(
          "pages.settings.inventory.uom.createTitle",
        ...
        footer={[
          ...
          {
            label: loading
              ? t("pages.settings.inventory.uom.creating", "Creating...")
              : t("pages.settings.inventory.uom.create", "Create UOM"),
            variant: "default",
            type: "submit",
            onClick: (e) => {
              e?.preventDefault();
                      ...
                      {baseUomOptions.length > 0 ? (
                        ...
                        <div className="px-2 py-1.5 space-y-2">
                          ...
                          {availableUoMs.length === 0 && (
                            ...
                            >
                              <Sparkles
                                className={cn(density.iconSize, "mr-1")}
                              />
                              {t(
                                "pages.settings.inventory.uom.initializeBaseUoms",
                                "Initialize Base UOMs"
                              )}
                            </Button>
                          )}
                        </div>
      ...
      <ConfirmationDialog
        ...
        title={t(
          "pages.settings.inventory.uom.confirmCreateTitle",
          "Confirm UOM Creation"
        )}
        description={
          <>
            <p className={cn("mb-3", density.textSizeSmall)}>
              {t(
                "pages.settings.inventory.uom.confirmCreateDescription",
                "Are you sure you want to create a new unit of measure with the following information?"
              )}
            </p>
        ...
        confirmAction={{
          label: loading
            ? t("pages.settings.inventory.uom.creating", "Creating...")
            : t(
                "pages.settings.inventory.uom.confirmCreate",
                "Confirm & Create"
              ),
      ...
      <ConfirmationDialog
        ...
        onOpenChange={(open) => {
          ...
          // Prevent ESC from closing FullScreenDialog when initialization dialog closes
          if (!open) {
            // Small delay to ensure event propagation is handled
            setTimeout(() => {
              // Focus back to the form to prevent FullScreenDialog from receiving ESC
        ...
        title={t(
          "pages.settings.inventory.uom.initializeTitle",
          "Initialize Base Units of Measure"
        )}
        description={
          <>
            <p className={cn("mb-3", density.textSizeSmall)}>
              {t(
                "pages.settings.inventory.uom.initializeDescription",
                "This will create the following base units of measure that can be used as reference points for other units:"
              )}
            </p>
            <div
              className={cn(
            ...
            >
              {getDefaultBaseUOMs().map((uom) => (
                <div
                  key={uom.code}
                  className={cn(
                    "flex justify-between items-center p-2 rounded-md bg-muted/50",
                    density.textSizeSmall
                  )}
                >
                  <div>
                    <span className="font-mono font-medium">{uom.code}</span>
        ...
        confirmAction={{
          label: isInitializing
            ? t("pages.settings.inventory.uom.initializing", "Initializing...")
            : t(
                "pages.settings.inventory.uom.confirmInitialize",
                "Initialize Base UOMs"
              ),
          onClick: handleInitializeBaseUOMs,
