"""Booking aggregate for Sales - Advanced CRUD with complex relationships."""
...
class Booking(AggregateRoot):
    """Represents a booking - advanced entity with complex relationships and business logic."""
    ...
    def update_details(
        ...
        *,
        ...
        """Update booking master data with validation."""
        if code is not None:
    ...
    def _validate_code(self, code: Optional[str]) -> Optional[str]:
        ...
        if not code or not code.strip():
            raise ValidationError("Booking code cannot be empty if provided")
        code = code.strip().upper()
        if len(code) > 100:
            raise ValidationError("Booking code cannot exceed 100 characters")
        return code
