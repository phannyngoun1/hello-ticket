...
class SQLVenueTypeRepository(VenueTypeRepository):
    """SQLModel implementation of VenueTypeRepository"""
        ...
        with self._session_factory() as session:
            ...
            if existing_model:
                ...
                try:
                    ...
                    session.rollback()
                    raise BusinessRuleError(f"Failed to update venue_type: {str(e)}")
            else:
                # Create new venue_type
                new_model = self._mapper.to_model(venue_type)
                ...
                try:
                    ...
                    session.rollback()
                    raise BusinessRuleError(f"Failed to create venue_type: {str(e)}")
    
    async def get_by_id(self, tenant_id: str, venue_type_id: str) -> Optional[VenueType]:
        """Get venue_type by ID (within tenant scope)"""
        with self._session_factory() as session:
            statement = select(VenueTypeModel).where(
                VenueTypeModel.id == venue_type_id,
                VenueTypeModel.tenant_id == tenant_id
            )
    ...
    async def delete(self, tenant_id: str, venue_type_id: str, hard_delete: bool = False) -> bool:
        """Delete a venue_type (soft-delete by default, hard-delete if specified)"""
        with self._session_factory() as session:
            statement = select(VenueTypeModel).where(
                VenueTypeModel.id == venue_type_id,
                VenueTypeModel.tenant_id == tenant_id
            ...
            try:
                ...
                session.rollback()
                raise BusinessRuleError(f"Failed to delete venue_type: {str(e)}")
